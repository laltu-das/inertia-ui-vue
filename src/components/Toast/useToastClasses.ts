...
import type { ToastAlign } from './types'
...
export function useToastClasses(props: UseToastClassesProps): UseToastClassesReturns {
    ...
    const wrapperClasses = computed(() => {
        ...
        if(props.divide.value) return twMerge(defaultWrapperClasses, 'dark:divide-gray-700 divide-x divide-gray-200', alignmentClass)
        return twMerge(defaultWrapperClasses, alignmentClass)
    })

    const contentClasses = computed(() => {
        if(props.type.value !== 'empty' && props.divide.value) return twMerge(defaultContentClasses, 'pl-3')
